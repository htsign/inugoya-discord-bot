...
import { UnexpectedIntensityError, intensityFromNumber, intensityFromNumberWithException } from './earthquake.js';
...
const subCommands = {
  register: {
    ...
    options: [
      ...
      {
        name: 'minintensity',
        description: `報告する最低の震度（デフォルト: ${DEFAULT_MIN_INTENSITY}）`,
  ...
  status: {
    ...
    async func(interaction) {
      ...
      if (configRecord != null) {
        const { channelId, minIntensity, createdAt, updatedAt } = configRecord;
